...
const PropertyCard = ({id, title,location,price,photo}:PropertyCardProps) => {
  return (
    ...
    >
      ...
      />
      <CardContent sx={{ display:'flex', flexDirection:'row',gap:"10px",paddingX:"5px"}}>
        <Stack direction="column" gap={2}> 
          <Typography fontSize={16} fontWeight={500} color="#11142d">{title}</Typography>
          <Stack direction="row" gap={0.5} alignItems="flex-start">
            <Place
              sx={{fontSize:10, color:"#11142d", marginTop:0.5}}
            />
        ...
        <Box px={1.5} py={0.5} borderRadius={1} bgcolor="#dadefa" height="fit-content">
          <Typography fontSize={12} fontWeight={600} color ="#475be8">{price}F</Typography>
        </Box>
      </CardContent>
    </Card>
  )
